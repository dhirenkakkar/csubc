...
private:
   ...
   */
   ...
   Node * buildTree(PNG * &im, int k); //builds the recursive toqutree

   int size(Node*&curr); //Helper for size method to recursively count num nodes

   PNG * render(Node * &curr); //recursive helper for render

   void prune(Node * curr, double tol); //helper for pruning to include the root

   bool leavesWithinTol(Node * curr, double tol);  //Prune helper to check if the leaves are withn tol


   PNG* makePNG(PNG * &im, pair<int,int>ul, pair<int,int>lr, int dim);  //recursive buildtree helper for children
